...
// New 分页对象构建器
// r__用来获取分页URL参数,默认是page,可通过config/pagination.go 修改
// db__Gorm查询句柄,用以查询数据集和获取总数
// baseURL_ 用于分页链接
// PerPage 每页条数,传参为小于或者等于0时为默认值10,可以通过config/pagination.go修改
func New(r *http.Request, db *gorm.DB, baseURL string, PerPage int) *Pagination {
	...
	if strings.Contains(baseURL, "?") {
		p.BaseURL = baseURL + "&" + config.GetString("pagination_url_query") + "="
	} else {
